...
def makeV(fileName, case, inputs_array, weight_array):
  if "Data" in fileName:
    hist = ROOT.TH2D(f'BpMST_dat_V', "BpM_vs_ST", Nbins_BpM, bin_lo_BpM, bin_hi_BpM, Nbins_ST, bin_lo_ST, bin_hi_ST)
  elif "Minor" in fileName:
    hist = ROOT.TH2D(f'BpMST_mnr_V', "BpM_vs_ST", Nbins_BpM, bin_lo_BpM, bin_hi_BpM, Nbins_ST, bin_lo_ST, bin_hi_ST)
  elif "Major" in fileName:
    hist = ROOT.TH2D(f'BpMST_pre_V', "BpM_ABDnn_vs_ST_ABCDnn", Nbins_BpM, bin_lo_BpM, bin_hi_BpM, Nbins_ST, bin_lo_ST, bin_hi_ST)
...
def makeHists2D(fileName, inputs_region, Bdecay_region, region, case):
  ...
  for i in range(len(inputs_array)):
    if inputs_array[i][0]>bin_lo_BpM and inputs_array[i][1]>bin_lo_ST:
      hist.Fill(inputs_array[i][0], inputs_array[i][1], weight_array[i])
  hist.Write()

  return inputs_array, weight_array
...
def main(fileName, tfileOption):
  ...
  for case in case_list:
    ...
    inputs_array , weight_array = makeHists2D(fileName, inputs_region, Bdecay_region, "D", case)

    #makeV(fileName, case, inputs_array , weight_array)
    #makeD2(fileName, case, inputs_array , weight_array)
    histFile.Close()
