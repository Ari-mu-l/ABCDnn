...
def smoothAndTruncate(hist_pre_pad, uncertType, case, region, yBinLowEdge, smooth): # uncertType: nom, corrUp/Dn, trainUp/Dn, pNetUp/Dn
    ...
    if smooth:
        if region=="D":
            if case=="case1" or case=="case2":
                hist_pre2D_out.Smooth(ntimes=1,option="k3a")
            else:
                hist_pre2D_out.Smooth(ntimes=1,option="k5a") #k5b
    ...
    #for region in ["D", "V", "B"]: # year-by-year gof
        ...
        if region=="D" and (case=="case1" or case=="case2"): # VR can be fully unblinded
            if case=="case1":
                ...
                #hist_tgt_partner, hist_pre_partner = getAlphaRatioTgtPreHists(histFileIn, 'V', 'case2') # fill the hole with B
